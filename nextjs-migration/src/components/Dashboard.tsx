...
import { Wallet, Download, Share2, DollarSign, Award, ArrowUpRight, TrendingUp, Clock, AlertCircle } from 'lucide-react';
...
export default function Dashboard({
  ...
  return (
    <div id="user-dashboard-root" className="space-y-6 font-sans">
      {/* Visual Welcome Banner */}
      <div className="relative overflow-hidden rounded-2xl bg-[#0b0c10] border border-zinc-200/50 dark:border-zinc-800/80 p-6 text-zinc-100 shadow-xl md:p-8">
        ...
        <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-5">
          <div>
            <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-3 py-1 text-xs font-semibold text-emerald-400 border border-emerald-500/20 backdrop-blur-xs">
          ...
          <button
            ...
            className="self-start md:self-center flex items-center gap-2 rounded-xl bg-white text-zinc-950 px-5 py-3 text-xs font-bold transition hover:bg-zinc-100 active:scale-95 shadow-md cursor-pointer"
          >
      ...
      {kycStatus === 'unsubmitted' && (
        <div className="flex items-start gap-4 rounded-xl border border-amber-200 bg-amber-50/70 p-4 text-amber-800 dark:border-amber-900/40 dark:bg-amber-950/20 dark:text-amber-305">
          <AlertCircle className="mt-0.5 h-5 w-5 shrink-0 text-amber-600" />
          <div className="flex-1">
            <h4 className="font-semibold text-sm">Identity Verification Required</h4>
            <p className="text-xs text-amber-750 dark:text-amber-400 leading-relaxed">
      ...
      {kycStatus === 'pending' && (
        <div className="flex items-center gap-3 rounded-xl border border-blue-200 bg-blue-50/70 p-4 text-blue-800 dark:border-blue-900/40 dark:bg-blue-950/20 dark:text-blue-305">
          <Clock className="h-5 w-5 text-blue-500 animate-pulse" />
          <div>
            <h4 className="font-semibold text-sm">KYC Verifications in Progress</h4>
            <p className="text-xs text-blue-750 dark:text-blue-400">
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-xs transition hover:shadow-sm dark:border-zinc-800/60 dark:bg-zinc-900">
          ...
          <div className="mt-4">
            ...
            <div className="mt-2 flex items-center gap-2 text-xs text-gray-500 dark:text-zinc-400">
              ...
              <span>•</span>
              <span className="rounded bg-teal-50 px-1.5 py-0.5 text-teal-700 dark:bg-teal-950/30 dark:text-teal-300">
                ₹{wallet.promo_balance.toFixed(2)} Bonus
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-xs transition hover:shadow-sm dark:border-zinc-800/60 dark:bg-zinc-900">
          ...
          <div className="mt-4">
            ...
            <p className="mt-2 text-xs text-indigo-600 dark:text-indigo-451 flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 rounded-2xl border border-gray-100 bg-white p-5 shadow-xs dark:border-zinc-850 dark:bg-zinc-900">
          ...
          <div className="mt-6 flex h-48 items-end gap-3 px-2">
            {[45, 60, 52, 78, 65, 88, 110].map((val, idx) => (
              <div key={idx} className="flex flex-1 flex-col items-center gap-2 group cursor-pointer">
                ...
                <div 
                  style={{ height: `${val}%` }}
                  className="w-full rounded-md bg-gradient-to-t from-teal-500 to-emerald-600 opacity-85 transition group-hover:opacity-100 shadow-xs"
                ></div>
        ...
        {/* Mini totals box */}
        <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-xs dark:border-zinc-850 dark:bg-zinc-900 flex flex-col justify-between">
          <div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-gray-900 dark:text-zinc-205">₹{totalEarnings.toFixed(2)} Volume</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="mt-6 pt-4 border-t border-gray-50 dark:border-zinc-800">
            <button 
              ...
              className="w-full rounded-xl bg-gray-50 dark:bg-zinc-800/60 hover:bg-gray-100 dark:hover:bg-zinc-800 py-2.5 text-xs font-semibold text-gray-700 dark:text-zinc-300 transition cursor-pointer"
            >
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-xs dark:border-zinc-850 dark:bg-zinc-900">
        ...
        {transactions.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-sm text-gray-400 font-sans">No account actions documented yet.</p>
          </div>
        ) : (
          <div className="overflow-x-auto font-sans">
            <table className="w-full text-left text-sm text-gray-500 dark:text-zinc-400">
              <thead className="bg-gray-55 text-xs uppercase text-gray-700 dark:bg-zinc-800/40 dark:text-zinc-300">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-zinc-800 font-sans">
                {transactions.slice(0, 5).map((tx) => (
                  <tr key={tx.id} className="hover:bg-gray-50/50 dark:hover:bg-zinc-800/20 font-sans">
                    <td className="px-4 py-3 font-sans">
                      <div className="font-semibold text-gray-950 dark:text-zinc-200">{tx.description}</div>
