...
export default function ChatSystem({ currentUser, usersList = [] }: ChatSystemProps) {
  ...
  return (
    ...
    >
      ...
      {isDragging && (
        <div className="absolute inset-0 bg-indigo-600/10 backdrop-blur-xs z-50 flex flex-col justify-center items-center text-indigo-600 dark:text-indigo-400 pointer-events-none border-4 border-dashed border-indigo-505 rounded-3xl animate-pulse">
      ...
      <div className={`w-full md:w-80 shrink-0 border-r dark:border-zinc-850 bg-slate-50/50 dark:bg-zinc-90 w/30 flex flex-col ${
        ...
        <div className="p-4 border-b dark:border-zinc-850 bg-white dark:bg-zinc-950 space-y-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3.5 py-2 text-xs rounded-xl bg-slate-50 dark:bg-zinc-900 font-semibold border border-slate-200 dark:border-zinc-805 outline-none focus:bg-white dark:focus:bg-zinc-900 text-slate-800 dark:text-zinc-100"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-slate-100/60 dark:divide-zinc-900/50 pb-16 md:pb-0">
          ...
          {searchQuery.trim() !== '' && (
            <div className="bg-indigo-50/25 dark:bg-indigo-950/20 p-2.5 border-b dark:border-zinc-850">
              ...
              {searchedUsersToStart.length === 0 ? (
                <span className="block text-[10.5px] text-zinc-400 dark:text-zinc-505 italic px-1">No user matched your query.</span>
              ) : (
                <div className="space-y-1">
                  {searchedUsersToStart.map(user => (
                    <button
                       ...
                       className="w-full text-left p-2 rounded-xl bg-white dark:bg-zinc-900 hover:bg-slate-50 dark:hover:bg-zinc-850 flex items-center justify-between border dark:border-zinc-800 transition duration-150 text-xs font-semibold select-none cursor-pointer"
                    >
                      <div className="flex gap-2 items-center min-w-0 font-sans">
                        <img referrerPolicy="no-referrer" src={user.avatar_url || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=150'} className="h-7 w-7 rounded-lg object-cover bg-slate-100 dark:bg-zinc-800" />
          ...
          {filteredRooms.length === 0 ? (
            <div className="py-12 text-center text-xs text-slate-400 dark:text-zinc-500 font-sans font-medium">
            ...
            filteredRooms.map((room) => {
              ...
              return (
                <div
                  ...
                  className={`p-3.5 flex gap-3 cursor-pointer transition-all duration-150 font-sans ${
                    isSel 
                ...
                >
                  <div className="relative shrink-0">
                    <img
                      ...
                      alt={room.name}
                      className="h-10 w-10 rounded-full object-cover bg-slate-100 dark:bg-zinc-900 border border-slate-100 dark:border-zinc-805"
                    />
                    {room.is_online && (
                  ...
                  <div className="flex-1 min-w-0 text-left space-y-0.5 pt-0.5">
                    ...
                    <div className="flex justify-between items-center text-[11px]">
                      <p className="text-slate-505 dark:text-zinc-400 truncate max-w-[155px] font-medium leading-relaxed">{room.last_message}</p>
                      {room.unread_count > 0 && (
                        <span className="h-4.5 w-4.5 rounded-md text-[9px] font-black bg-indigo-600 dark:bg-indigo-505 text-white flex items-center justify-center shrink-0">
                          {room.unread_count}
                        </span>
                      )}
                    </div>
      ...
      <div className={`flex-grow flex flex-col bg-white dark:bg-zinc-950 min-w-0 ${
        ...
        {activeRoom ? (
          <>
            <div className="px-5 py-3 border-b border-slate-100 dark:border-zinc-900 flex items-center justify-between shrink-0 bg-slate-50/50 dark:bg-zinc-900/20 font-sans">
              ...
              <div className="flex items-center gap-1 p-0.5 bg-white dark:bg-zinc-900 border dark:border-zinc-800 rounded-xl">
                <span className="h-2 w-2 rounded-full bg-emerald-500 text-transparent mx-2 inline-block animate-pulse font-sans">●</span>
            ...
            {validationError && (
              <div className="px-4 py-2.5 bg-rose-50 dark:bg-rose-950/25 border-b border-rose-100 dark:border-rose-900/30 text-[11px] text-rose-600 dark:text-rose-400 flex items-center gap-2 font-bold animate-fadeIn shrink-0 font-sans">
            ...
            <div className="flex-grow overflow-y-auto p-4 md:p-5 space-y-4 bg-slate-50/10 dark:bg-zinc-900/10 font-sans">
                  ...
                  <div key={msg.id || index} className={`flex ${isMe ? 'justify-end' : 'justify-start'} animate-fadeIn`}>
                    <div className="max-w-[75%] space-y-1">
                      ...
                      <div className={`p-3.5 rounded-3xl text-xs font-semibold leading-relaxed relative group ${
                        isMe 
                          ? 'bg-indigo-600 dark:bg-indigo-505 text-white rounded-br-none' 
                        ...
                        {msg.attachment && (
                          <div className="mt-1.5 space-y-1">
                            ...
                            {msg.attachment.type === 'video' && (
                              <div className="border border-slate-100 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-xs bg-slate-50 dark:bg-zinc-955 max-w-64">
                                <video 
                                  controls 
                                  src={msg.attachment.url} 
                            ...
                            {msg.attachment.type === 'document' && (
                              <div className="flex items-center gap-3 bg-red-50 dark:bg-red-955/20 border border-red-100 dark:border-red-900/30 text-red-750 dark:text-red-400 rounded-2xl p-3 max-w-64">
                            ...
                            {msg.attachment.type === 'file' && (
                              <div className="flex items-center gap-3 bg-slate-100 dark:bg-zinc-900 border dark:border-zinc-800 text-slate-705 dark:text-zinc-300 rounded-2xl p-3 max-w-64">
                        ...
                        <div className="absolute top-1/2 -translate-y-1/2 hidden group-hover:flex gap-1 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-md rounded-xl p-1 border dark:border-zinc-800 shadow-md z-15 right-full mr-2 select-none">
                        ...
                        {msg.reactions && msg.reactions.length > 0 && (
                          <div className="flex flex-wrap gap-1 absolute top-full -translate-y-1/2 right-4 bg-white dark:bg-zinc-900 px-1.5 py-0.5 rounded-full border dark:border-zinc-800 text-[9.5px] text-slate-505 shadow-sm z-12 select-none">
                            {msg.reactions.map((react, r_i) => (
                              <span key={r_i} className="cursor-pointer font-sans" onClick={() => handleMessageReaction(msg.id, react)}>{react}</span>
                            ))}
                          </div>
                        )}
                      </div>
            ...
            <div className="p-3 bg-white dark:bg-zinc-950 border-t border-slate-100 dark:border-zinc-900 space-y-2 font-sans">
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="rounded-xl h-10 w-10 bg-slate-50 hover:bg-slate-100 dark:bg-zinc-900 dark:hover:bg-zinc-850 border border-slate-202 dark:border-zinc-800 text-slate-505 dark:text-zinc-400 flex items-center justify-center transition shrink-0 cursor-pointer"
                ...
                >
                  <input
                    ...
                    className="flex-1 rounded-2xl bg-slate-50 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800/80 p-2.5 text-xs font-semibold focus:bg-white dark:focus:bg-zinc-950 focus:outline-none focus:ring-1 focus:ring-indigo-505 text-slate-850 dark:text-zinc-150"
                  />
                  ...
                  <button
                    ...
                    className={`rounded-xl h-10 w-10 border flex items-center justify-center transition shrink-0 cursor-pointer ${
                      showEmojiPicker 
                        ...
                        : 'bg-slate-50 hover:bg-slate-100 dark:bg-zinc-90 w dark:hover:bg-zinc-850 border-slate-205 dark:border-zinc-800 text-slate-500 dark:text-zinc-400'
                    }`}
                  ...
                  <button
                    type="submit"
                    className="rounded-2xl h-10 w-10 bg-indigo-600 hover:bg-indigo-700 text-[#FFFFFF] shadow-sm flex items-center justify-center transition shrink-0 hover:scale-105 active:scale-95 cursor-pointerborder-0"
                  >
              ...
              <div className="flex items-center gap-1 px-1.5 py-0.5 border-t border-slate-50 dark:border-zinc-900/40 justify-between font-sans">
                ...
                <span className="text-[9.5px] text-slate-400 dark:text-zinc-500 font-medium italic hidden sm:inline">Drag Files Here to Upload</span>
              </div>

              {/* Advanced inline picker: Show emoji picker directly below the message input area */}
              {showEmojiPicker && (
                <div className="border border-slate-100 dark:border-zinc-800 rounded-2xl bg-slate-50 dark:bg-zinc-900/50 p-3.5 space-y-3 animate-fadeIn font-sans">
                  ...
                  <div className="flex gap-1 border-b pb-2 dark:border-zinc-800">
                    {ALL_EMOJIS_CATEGORIES.map((cat, c_idx) => (
                      <button
                        ...
                        className={`px-2.5 py-1 text-[10px] rounded-lg font-bold transition cursor-pointer border-0 ${
                          activeEmojiCategory === c_idx 
                            ? 'bg-indigo-600 text-white' 
                            : 'bg-white hover:bg-slate-50 dark:bg-zinc-800 text-slate-600 dark:text-zinc-400 hover:text-slate-900'
                        }`}
                      >
                        {cat.name}
                      </button>
                  ...
                  <div className="grid grid-cols-10 gap-1.5 max-h-24 overflow-y-auto pr-1">
                    {ALL_EMOJIS_CATEGORIES[activeEmojiCategory].emojis.map((emoji) => (
                      <button
                        key={emoji}
                        onClick={() => handleSelectEmoji(emoji)}
                        className="py-1 text-center hover:scale-130 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-lg transition text-base font-sans select-none cursor-pointer border-0"
                      >
                        {emoji}
                      </button>
                    ))}
          ...
          <div className="flex-1 flex flex-col justify-center items-center text-center p-12 text-slate-405 dark:text-zinc-505 selection:none select-none font-sans">
